...
from graph_search import create_grid_and_edges, find_path 
...
class MapData:
    def __init__(self, map_file_name):
        ...
        # Define a grid for a particular altitude and safety margin around obstaclesself.grid_2, self.edges, self.north_offset, self.east_offset = create_grid_and_edges(self.map_data, self.TARGET_ALTITUDE, self.SAFETY_DISTANCE)
    ...
    def search_path(self):
        ...
        def onclick(event):
            ...
            if len(coords) == 2:
                ...
                path2, _2 = find_path(self.grid, self.edges, grid_start, grid_goal)
                print("PATH2 {0}".format(len(path2)))
                print([[p[0], p[1] , self.TARGET_ALTITUDE, 0] for p in path2])

                if len(path2) > 0:
                    # plot path
                    print('Plotting the path ...')
                    waypoints = [[p[0], p[1] , self.TARGET_ALTITUDE, 0] for p in path2]
                    ...
                    waypoints = [[p[0] + self.north_offset, p[1] + self.east_offset, self.TARGET_ALTITUDE, 0] for p in path2]
                    ## Set self.waypoints
...
if __name__ == "__main__":
    ...
    m = MapData('/home/darshan/udacity/Motion planning/6.Project/githubRepos/FCND-Motion-Planning-master/colliders.csv')
    m.search_path()
